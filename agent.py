...
def build_system_prompt(allowed_window: str) -> str:
    ...
    return (
        ...
        "\n"
        "3. `find_text` — click a UI label by name on MAC-NATIVE apps only.\n"
        "   - Do NOT use for TeamViewer/nCara — TeamViewer content is compressed video,\n"
        "     OCR cannot read it. Use computer (screenshot → see → click) for nCara instead.\n"
        "   - Valid use: finding text in Mac-native windows (Finder, Preview, Safari).\n"
        "\n"
